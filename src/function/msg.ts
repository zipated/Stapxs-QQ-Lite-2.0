    ...
    notify: (name: string, msg: { [key: string]: any }) => {
        ...
        if(groupId == runtimeData.chatInfo.show.id) {
            ...
            info.forEach((item: any) => {
                switch(item.type) {
                    case 'img':  str += `<img src="${item.src}"/>`; break
                    case 'nor': str += item.txt; break
                    case 'qq': {
        ...
        runtimeData.messageList.forEach((item, index) => {
            if (item.message_id === msgId) {
                if(runtimeData.messageList[index].emoji_like) {
                    // 寻找有没有 emoji_id 相同的
                    let hasAdd = false
                    runtimeData.messageList[index].emoji_like.forEach((item: {emoji_id: number, count: number}) => {
                        if(item.emoji_id == id) {
                            item.count++
                            hasAdd = true
                        }
                    ...
                    if(!hasAdd) {
                        runtimeData.messageList[index].emoji_like.push({emoji_id: id, count: 1})
                    }
                } else {
                    runtimeData.messageList[index].emoji_like = [{emoji_id: id, count: 1}]
                }
            }
        })
    },
